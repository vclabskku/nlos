...
#define MAX_INTERFACES 10                  ///< Size of array used to hold information about interfaces
#define MAX_CONNECTION_STRING_LENGTH 256   ///< Maximum length of a connection string
...
/*
 * Forward declaration of struct Radar_t
 */
...
struct Radar_t_
{
    ...
    int (CALL_CONV *interfaceRegisterCallbacks)(RadarHandle_t radarHandle);
 ...
 *  - CSV or binary - 0x0001 is CSV, 0x0002 is binary
 *  - Raw or normalized values - 0x0010 is raw, 0x0020 is normalized
 *
 * @param[in] radarHandle A pointer to an initialized RadarHandle_t (radarHandle must be initialized by calling \ref NVA_InitHandle)
 * @param[in] fileName    File name for log file.
 * @param[in] mode        Mode of logging.
 * @param[in] precision   number of decimals in CSV or number of bytes (4/9) in binary format.
 * @return Status of the operation. NVA_SUCCESS if all is OK, @see NVA_ErrorCodes.h for more information about error codes.
 * @ingroup misc_funcs
 */
